...
S3_KEY_PATH_MANIFEST_FILE = 'cactus/with_with_table_800/custom_labels.json'
S3_KEY_PATH_IMAGES = 'assets/cactus/with_with_table_800/'
LOCAL_PATH = 'rekognition/dataset_gen/dataset/'
LOCAL_IMAGES_PATH = 'rekognition/dataset_gen/dataset/images/'
COCO_MANIFEST = 'annotation.json'
JOB_NAME = 'cactus_with_table_800'
CL_MANIFEST_FILE = 'custom_labels.manifest'

# S3 location for images
s3_bucket = S3_BUCKET
s3_key_path_manifest_file = S3_KEY_PATH_MANIFEST_FILE
s3_key_path_images = S3_KEY_PATH_IMAGES
...
coco_manifest = COCO_MANIFEST
coco_json_file = local_path + coco_manifest
job_name = JOB_NAME
cl_manifest_file = CL_MANIFEST_FILE
...
images_dict = {image['id']: cl_json_line(
    label_attribute, image) for image in images}
...
for annotation in annotations:
    ...
    for category in categories:
        if annotation['category_id'] == category['id']:
            getattr(image, label_attribute +
                    '-metadata')['class-map'][category['id']] = category['name']
...
print('Uploading' + local_path + cl_manifest_file +
      ' to ' + s3_key_path_manifest_file)
print(s3_bucket)
s3 = boto3.resource('s3')
...
print('\033[1m s3://' + s3_bucket + '/' +
      s3_key_path_manifest_file + cl_manifest_file + '\033[0m')
